...
android::SoftOMXComponent* SoftFFmpegVideo::createSoftOMXComponent(
    ...
    for (size_t i = 0; i < kNumVideoComponents; ++i) {
        if (!strcasecmp(name, android::kVideoComponents[i].mName)) {
            componentRole = strdup(android::kVideoComponents[i].mRole);
            codingType = android::kVideoComponents[i].mVideoCodingType;
            codecID = android::kVideoComponents[i].mCodecID;
            break;
        }
    }
