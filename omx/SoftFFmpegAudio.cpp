...
OMX_ERRORTYPE SoftFFmpegAudio::isRoleSupported(
    ...
    for (size_t i = 0;
         i < sizeof(kAudioComponents) / sizeof(kAudioComponents[0]);
         ++i) {
...
android::SoftOMXComponent* SoftFFmpegAudio::createSoftOMXComponent(
    ...
    for (size_t i = 0; i < kNumAudioComponents; ++i) {
        if (!strcasecmp(name, android::kAudioComponents[i].mName)) {
            componentRole = strdup(android::kAudioComponents[i].mRole);
            codingType = android::kAudioComponents[i].mAudioCodingType;
            codecID = android::kAudioComponents[i].mCodecID;
            break;
         }
     }
