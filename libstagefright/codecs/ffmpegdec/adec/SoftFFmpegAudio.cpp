...
void SoftFFmpegAudio::adjustAudioParams() {
    ...
    ALOGD("Original audio params: channels=%d sampling_rate=%d sample_format=%d",
            mCtx->channels, mCtx->sample_rate, mCtx->bits_per_coded_sample);
    ...
    if (mHighResAudioEnabled) {
        max_rate = 192000;
        if ((mCtx->bits_per_coded_sample >= 24) ||
    ...
    mAudioSrcFmt = mAudioTgtFmt = sample_fmt;
    mAudioSrcChannelLayout = mAudioTgtChannelLayout =
        av_get_default_channel_layout(channels);

    ALOGD("Adjusted audio params: channels=%d sampling_rate=%d sample_format=%d",
            channels, sampling_rate, sample_fmt);
}
